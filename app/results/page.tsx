...
const moodGoogleTypes: Record<string, string[]> = {
  Peaceful: ["park", "library", "book_store", "spa", "natural_feature"],
  Adventurous: [
    "tourist_attraction",
    "amusement_park",
    "hiking_area",
  ...
  Social: ["cafe", "bar", "restaurant", "night_club", "shopping_mall"],
  Sad: ["cafe", "book_store", "art_gallery", "museum", "church"],
  Hungry: ["restaurant", "cafe", "bakery", "meal_takeaway", "food"],
  Energetic: ["gym", "park", "stadium", "sports_complex", "bicycle_store"],
  Creative: [
    "art_gallery",
    "museum",
    "library",
  ...
  Relaxed: ["spa", "park", "cafe", "beauty_salon", "garden"],
  Trip: [
    "tourist_attraction",
    "natural_feature",
    "campground",
  ...
  Inspired: ["museum", "church", "art_gallery", "library", "historic_site"],
  Romantic: [
    "restaurant",
    "cafe",
    "park",
...
export default function ResultsPage() {
  ...
  const fetchPlaces = async (
    ...
    try {
      ...
      console.log(
        `Searching for place types: ${types.join(", ")} within ${radius}m`
      );
              ...
              service.nearbySearch(
                ...
                (results, status) => {
                  if (
                    ...
                    console.log(
                      `Found ${results.length} places for type: ${type}`
                    );
                    resolve(results);
                  } else if (
                    status ===
        ...
        service.getDetails(
          ...
          (result, status) => {
            if (
              ...
              console.log(
                `Failed to get details for place: ${
    ...
    try {
      ...
      try {
        ...
        const uniquePlaces = Array.from(
          new Map(rawPlaces.map((place) => [place.place_id, place])).values()
        );
        ...
        const detailedPlaces = (
          await Promise.all(detailedPlacesPromises)
        ...
        if (detailedPlaces.length === 0) {
          if (!usedFallback) {
            ...
            throw {
              type: "no_results",
              message:
                "No places found with mood-specific types - trying fallback types",
              retryable: true,
            };
        ...
        if (!usedFallback) {
          ...
          throw {
            type: "api",
            message:
              "Failed to fetch places with mood types - trying fallback types",
            retryable: true,
          };
        } else {
          throw {
            type: "api",
            message:
              err instanceof Error ? err.message : "Failed to fetch places",
            retryable: true,
          };
        }
      }
  ...
  const filteredAndSortedPlaces = useMemo(() => {
      ...
      ? places.filter((place) => {
          ...
          if (["Rain", "Snow"].includes(weatherType || "")) {
            return !place.types?.some((t) =>
              outdoorTypes.some((type) => t.includes(type))
            );
          }
          return true;
        })
    ...
    switch (sortOption) {
      ...
      default:
        ...
        sorted.sort(
          (a, b) => calculateQualityScore(b) - calculateQualityScore(a)
        );
        break;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
        ...
        <p className="text-muted-foreground text-center mb-6">
          We need your location to provide recommendations.
        </p>
        <Link
          href="/"
